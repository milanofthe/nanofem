...
sign carrying expressions elsewhere. Second order elements inherit the same
property, since their face functions are pinned by the sorted vertex triple
of each face.
\end{decision}
...
\begin{fieldnote}{a smaller separator that increases fill}
A refinement is to take the smaller of the two candidate boundary
layers as the separator rather than always the second. Implemented and measured, it produced \(0.47\)
million nonzeros against \(0.46\), and was removed.
\end{fieldnote}
...
against the wavelength. Equilibration gains a factor of about six below
\(1\) GHz and loses about a factor of three between \(1\) and \(5\) GHz.}
\label{fig:cond}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Runtime by stage on the reference antenna. The factorization
dominates, so the ordering was measured and the assembly was not
optimized.}
\label{tab:time}
\end{table}
...
\begin{fieldnote}{an orientation error that the value tests miss}
This test is absent from table~\ref{tab:valid} because the second order code
it used is not in the current file. The accumulated phase error of the TEM
line was measured against its exact value \(-k_0 L\) at several
frequencies. That error grows as \(k^{2p+1}\), so tripling the frequency
multiplies it by about \(27\) at first order and \(243\) at second.
Table~\ref{tab:order} gives the measurement. A wrong orientation or a
mismatched face function still produces plausible scattering parameters, but
changes the rate.
\end{fieldnote}
...
Second order elements were implemented and validated by the convergence rate
of table~\ref{tab:order} before being removed. They carry twenty unknowns
per tetrahedron instead of six: the six Whitney functions, six curl free
edge gradients, and two functions per face. At six cells per wavelength they
are three orders of magnitude more accurate than first order on the same
mesh. They also consume the remaining budget and multiply the number of
unknowns by five. The code is kept on a branch.
...
Three decisions account for most of what fits. Sorting the mesh vertices
removes orientation from the solver. Four frequency coefficients per matrix
entry keep the assembly at one pass for an entire sweep. Storing the layer
stretch separately from the permittivity lets conductivity reuse the mass
entry. Each removes bookkeeping rather than compressing it.
