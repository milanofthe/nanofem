...
\begin{tikzpicture}[scale=1.0, font=\footnotesize,
  ed/.style={line width=0.5pt, ->, >=stealth, shorten >=2pt, shorten <=2pt},
  hd/.style={line width=0.4pt, dashed, ->, >=stealth, shorten >=2pt, shorten <=2pt}]
\coordinate (v0) at (0,0);
\coordinate (v1) at (4.2,0.15);
\coordinate (v2) at (1.15,0.95);
\coordinate (v3) at (2.6,2.6);
\draw[ed] (v0) -- (v1);
\draw[hd] (v0) -- (v2);
\draw[ed] (v0) -- (v3);
\draw[hd] (v1) -- (v2);
\draw[ed] (v1) -- (v3);
\draw[hd] (v2) -- (v3);
\foreach \i in {0,1,2,3} \fill (v\i) circle (1.6pt);
\node[below left] at (v0) {$n_0$};
\node[below right] at (v1) {$n_1$};
\node[below right, inner sep=1pt] at (v2) {$n_2$};
\node[above] at (v3) {$n_3$};
\node[below] at ($(v0)!0.5!(v1)$) {$\mathbf{W}_{01}$};
\node[left] at ($(v0)!0.68!(v3)$) {$\mathbf{W}_{03}$};
\node[right] at ($(v1)!0.55!(v3)$) {$\mathbf{W}_{13}$};
\end{tikzpicture}
\caption{The six edge unknowns of a tetrahedron. The unknown on an edge is
the line integral of \(\mathbf{E}\) along it. Node indices are sorted on
input, so every arrow runs from the lower to the higher global index and two
tetrahedra sharing an edge assign it the same direction without a sign
convention. Dashed edges are behind the front face.}
\label{fig:tet}
\end{figure}
