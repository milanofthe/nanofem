...
resonates at \(2.40\) GHz on its default mesh and at \(2.48\) GHz on a mesh
refined by a factor of two. Mesh convergence has to be established by
...
\begin{axis}[bwaxis, grid=none, width=0.44\textwidth, height=0.40\textwidth,
  xlabel={$x$ / mm}, ylabel={$y$ / mm},
  title={\footnotesize substrate, $z = h/2$, floor $-20$ dB},
  enlargelimits=false, axis on top, point meta min=0, point meta max=1,
  colormap={grays}{gray(0cm)=(1); gray(1cm)=(0.86); gray(2cm)=(0.70); gray(3cm)=(0.52); gray(4cm)=(0.30); gray(5cm)=(0.08)},
  colormap access=piecewise constant,]
\addplot[matrix plot*, point meta=explicit, mesh/cols=44]
  table[x=x, y=y, meta=e] {data/field_xy.dat};
\draw[line width=0.7pt] (axis cs:20.8,31.6) rectangle (axis cs:69.2,68.4);
\draw[line width=1.1pt] (axis cs:43,31.6) -- (axis cs:47,31.6);
\end{axis}
...
\begin{axis}[bwaxis, grid=none, width=0.44\textwidth, height=0.40\textwidth,
  ...
  colormap access=piecewise constant,]
\addplot[matrix plot*, point meta=explicit, mesh/cols=44]
  table[x=x, y=y, meta=e] {data/field_xz.dat};
\draw[line width=1.1pt] (axis cs:22,0) -- (axis cs:78,0);
\draw[line width=1.1pt] (axis cs:31.6,1.57) -- (axis cs:68.4,1.57);
\end{axis}
\end{tikzpicture}
\caption{Magnitude of the electric field at \(2.4\) GHz with the port driven,
on six grey levels of a decibel scale relative to the peak of each cut. Left,
a horizontal cut through the middle of the substrate with the patch outline
and the feed drawn: the field is confined under the patch and forms the half
wave variation along \(y\) that sets the resonant length, with maxima at the
two radiating edges and a null across the middle. Right, a vertical cut along the
resonant direction through the middle of the patch, with the ground plane and
the patch as the heavy lines: the same half wave appears in the substrate,
and the field that escapes past the two patch edges is the fringing that
radiates.}
\label{fig:field}
\end{figure}
...
97 percent of the work per frequency, so the fill of the ordering sets the
