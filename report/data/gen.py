...
def edges(box):
    """The twelve edges of a box, as pairs of points."""
    c = corners(box)
    return [(c[i], c[j]) for i in range(8) for j in range(i + 1, 8)
            if sum(1 for k in range(3) if c[i][k] != c[j][k]) == 1]


def about(segs, ax, at, near):
    """Segments cut at an axis aligned plane, as behind and in front of it.

    A segment crossing the plane is split there, so each piece lies wholly
    on one side. Anything lying in the plane counts as in front, since that
    is where it is drawn.
    """
    back, front = [], []
    for a, b in segs:
        da, db = a[ax] - at, b[ax] - at
        if da * db < 0.0:
            t = da / (da - db)
            m = tuple(a[k] + t * (b[k] - a[k]) for k in range(3))
            parts = [(a, m, da), (m, b, db)]
        else:
            parts = [(a, b, da if da != 0.0 else db)]
        for q, r, d in parts:
            if d == 0.0 or (d > 0.0) == near:
                front.append((q, r))
            else:
                back.append((q, r))
    return back, front
...
def chain(pts):
    """A polyline as its individual segments."""
...
def draw(D, segs, w=0.3, opt=""):
    return ["\\draw[line width=%.2fpt%s] %s -- %s;" % (w, opt, D(a), D(b))
            for a, b in segs]
...
def cut3d(name, box, width, ax, at, plane, to3d, segs, marks):
    """One cut plane inside the domain outline, with its contours on it.

    The plane is opaque, so everything else is split along it: what lies
    behind is drawn first and the plane covers it, what lies in front is
    drawn after and stays visible. That holds for the box edges as much as
    for the metal, so the plane sits inside the box rather than on top of
    it. The contours are computed in the two dimensions of the plane
    itself, so the lines are exact.
    """
    depth, D = scene(box, width)
    e = [0.0, 0.0, 0.0]
    e[ax] = 1.0
    near = depth(tuple(e)) > 0.0
    beb, bef = about(edges(box), ax, at, near)
    mab, maf = about([s for m in marks for s in chain(m)], ax, at, near)
    L = draw(D, beb, 0.25, ", black!40") + draw(D, mab, 0.8)
    L.append(filled(D, plane))
    L += draw(D, [(to3d(a), to3d(b)) for a, b in segs])
    L += draw(D, maf, 0.8) + draw(D, bef, 0.25, ", black!40")
    write(name, L)
    return len(segs)
