...
The linear algebra is the largest block. That is the opposite of nanospice,
where the parser dominated: a field solver has a trivial input format and a
solver whose cost grows with the cube of the mesh resolution. Everything the
budget excluded is listed with reasons in section~\ref{sec:cuts}.
...
formulation, which is a perfect magnetic wall. That is a useful default: a
symmetry plane of the magnetic type costs nothing at all, not even a card in
the deck.
...
spurious modes appear. The cost is six unknowns per tetrahedron instead of
twelve, so the choice is cheaper as well as correct.
\end{decision}
...
\section{Orientation, and why the solver has no signs}
...
direction of every shared edge by construction. The signs disappear from the
solver entirely, not just from one loop: assembly, the port excitation
integral and the field export are all sign free. This is one line in the
reader against roughly ten scattered through the rest of the file, and it
removes a class of bug that produces plausible looking output rather than an
obvious failure. It is also what made second order elements reachable later,
because the orientation of face functions became free as well.
\end{decision}
...
\(-30.7\) dB in the test suite.

\section{Loss, and one assembly for the whole sweep}
...
and the boundary terms as \(+j k_0\). If that were the whole story the
matrix would be a quadratic polynomial in \(k_0\) and the assembly could run
once for an entire sweep. Two of the three loss mechanisms preserve this,
and the third extends it by exactly one term.
...
then four multiplications per nonzero. The assembly, which walks every
tetrahedron and every boundary face and touches the mesh, runs exactly once
no matter how many frequencies the sweep contains. This is the decision that
buys the most in this file: it keeps frequency dependent physics and a
one-time assembly at the same time, and it costs one array index rather than
a rebuild.
\end{decision}
...
Iterative solvers are the standard choice at this problem size, but the
curl-curl operator defeats the simple ones. Its large gradient nullspace
makes Krylov methods with a diagonal preconditioner converge arbitrarily
slowly, and the preconditioners that do work, auxiliary space methods or
algebraic multigrid on the edge space, are far larger than the entire
budget. A direct factorization has a predictable cost and no convergence
parameters. The price is memory: the practical ceiling is a few tens of
...
gives \(0.46\) million nonzeros in \(L\) against \(3.55\) million in the
natural order, a factor of \(7.7\).

\begin{fieldnote}{a fill heuristic that reads better than it measures}
An obvious refinement is to take the smaller of the two candidate boundary
layers as the separator rather than always the second. Implemented and
measured, it produced \(0.47\) million nonzeros, slightly worse, and was
removed. The general lesson holds for the whole file: fill reducing
heuristics are cheap to reason about and cheap to be wrong about, so they
...
resonance. Bunch-Kaufman pivoting is the textbook remedy and does not fit
the budget. Two cheaper measures cover most of the ground.
...
against the wavelength. Equilibration gains a factor of about six where it
matters.}
\label{fig:cond}
\end{figure}
...
residual by about a factor of four. The larger benefit is that the residual
is measured rather than assumed, and both it and the pivot spread are
printed at the end of a run.

\begin{decision}{report the residual instead of trusting the factorization}
A direct solver that returns a vector says nothing about how well it solved.
Since the residual costs one matrix vector product once refinement is
present, the solver reports the worst relative residual over the sweep. A
user then knows whether a run is trustworthy without having to reason about
conditioning, and the number is small enough to print on one line.
\end{decision}
...
The reference model is an edge fed microstrip patch antenna on a
\(1.57\) mm substrate of \(\varepsilon_r = 2.2\), meshed by Gmsh into
\(3760\) tetrahedra and \(4215\) unknowns.
...
\end{axis}
\end{tikzpicture}
\caption{Reflection of the reference antenna. The resonance appears as a dip
near \(2.4\) GHz on this mesh. It is not converged: refining the mesh moves
it to \(2.48\) GHz, and part of the remaining drift is the first order
absorbing boundary, which sits about a third of a wavelength above the
patch.}
\label{fig:patch}
\end{figure}
...
stage & time & share \\
\midrule
mesh, assembly and matrix build & 7.3 ms & once \\
symbolic analysis & 1.5 ms & once \\
numeric factorization & 46.5 ms & per frequency \\
triangular solve & 1.0 ms & per frequency and port \\
\bottomrule
\end{tabular}
\caption{Where the time goes on the reference antenna. The factorization is
the whole cost, which is why the ordering was measured and the assembly was
not optimized.}
\label{tab:time}
\end{table}
...
\begin{fieldnote}{a wrong orientation still produces plausible parameters}
The sharpest test in the project was not in this list. During an experiment
with second order elements, the accumulated phase error of the TEM line was
measured against its exact value \(-k_0 L\) at several frequencies. Theory
says that error grows as \(k^{2p+1}\), so tripling the frequency must
multiply it by about \(27\) at first order and \(243\) at second.
Table~\ref{tab:order} shows what was measured. A wrong orientation or a
mismatched face function still produces plausible looking scattering
parameters, but it destroys the rate, which is why a rate is worth more as a
test than a value.
\end{fieldnote}
...
\begin{tabular}{rrr}
\toprule
cells per wavelength & phase error, \(p=1\) & phase error, \(p=2\) \\
\midrule
30.0 & \(1.63 \times 10^{-3}\) & \(5.19 \times 10^{-8}\) \\
10.0 & \(4.53 \times 10^{-2}\) & \(1.42 \times 10^{-5}\) \\
6.0 & \(2.22 \times 10^{-1}\) & \(2.25 \times 10^{-4}\) \\
3.7 & \(1.04 \times 10^{0}\) & \(3.57 \times 10^{-3}\) \\
\bottomrule
\end{tabular}
\caption{Accumulated phase error in radians over the reference line. The
measured growth per factor three in frequency was \(27.8\) at first order
and \(274\) at second, against \(27\) and \(243\) from theory.}
\label{tab:order}
\end{table}
...
\midrule
second order elements & 150 to 190 & built, measured, removed \\
modal waveguide ports & 200+ & needs a 2D eigensolver \\
supernodal factorization & 200+ & the only remaining speed lever \\
...
\bottomrule
\end{tabular}
\caption{What the budget excluded.}
\end{table}

Second order elements are the interesting entry. They were implemented,
validated by the convergence rate of table~\ref{tab:order}, and then
removed. They work: twenty unknowns per tetrahedron instead of six, the six
Whitney functions plus six curl free edge gradients plus two functions per
face, and the vertex sorting of the mesh reader made their orientation free.
At six cells per wavelength they are three orders of magnitude more accurate
than first order on the same mesh. They also consume the entire remaining
budget and multiply the number of unknowns by five, and the project is worth
more with room left for the rest of the physics. The code is kept on a
branch.

\section{Closing}

The parts of this solver that matter are not large. The formulation is one
equation, the element matrices are closed form, the boundary conditions are
one face mass matrix with four different weights, and the port is a sheet
impedance. What the budget mainly bought was the discipline to find the
decisions that make those parts small: sorting the mesh vertices so that
orientation never appears, storing four frequency coefficients so that
assembly happens once, and keeping one tensor for the layer stretch so that
conductivity reuses the mass entry. Each of those is worth more than the
lines it saves, because it removes a category of bookkeeping rather than
compressing it.

The measurements matter as much. The separator refinement that looked
obviously better and was not, the equilibration that helps by a factor of
six at low frequency and slightly hurts in band, and the second order
elements that work and were still cut are all decisions that could have gone
the other way on argument alone. They went the way the numbers pointed.
