...
resonates at \(2.48\) GHz on its default mesh and at \(2.50\) GHz on a mesh
...
\(10668\) tetrahedra and \(11292\) unknowns.
...
opaque faces. The thin layer at the bottom is the substrate, the block above
it is the air region, and the shaded ring and cap are the perfectly matched
layer. The element size follows a distance field on the patch and the feed,
growing from \(3\) mm at the metal to \(9\) mm at the outer corners, which
keeps the aspect ratio bounded through the transition out of the
\(1.57\) mm substrate. A uniform maximum size instead produces slivers
spanning the air region and moves the resonance by two percent.}
\label{fig:mesh}
\end{figure}
...
at \(2.48\) GHz on this mesh, against a design value of \(2.45\) GHz.
Refining the mesh moves it to \(2.50\) GHz, so the discretization error is
...
\begin{axis}[bwaxis, grid=none, width=0.45\textwidth, height=0.42\textwidth,
  ...
  title={\footnotesize substrate, $z = h/2$}, enlargelimits=false]
\addplot[black, line width=0.35pt] table[x=x, y=y] {data/cont_xy.dat};
\addplot[black, line width=0.8pt] coordinates
  {(20.8,31.6) (69.2,31.6) (69.2,68.4) (20.8,68.4) (20.8,31.6)};
\end{axis}
\end{tikzpicture}\hfill
\begin{tikzpicture}[font=\footnotesize, baseline]
\begin{axis}[bwaxis, grid=none, width=0.45\textwidth, height=0.42\textwidth,
  xlabel={$y$ / mm}, ylabel={$z$ / mm},
  title={\footnotesize cut at $x = 45$ mm}, enlargelimits=false]
\addplot[black, line width=0.35pt] table[x=x, y=y] {data/cont_xz.dat};
\addplot[black, line width=1.0pt] coordinates {(22,0) (78,0)};
\addplot[black, line width=1.0pt] coordinates {(31.6,1.57) (68.4,1.57)};
\end{axis}
\end{tikzpicture}
\caption{Contours of the electric field magnitude at the resonance, at
\(-11\), \(-7\), \(-4\) and \(-2\) dB below the peak of each cut. Left, a
horizontal cut through the middle of the substrate with the patch outline
drawn: the field is confined under the patch and forms the half wave along
\(y\) that sets the resonant length, with maxima at the two radiating edges
and a null across the middle. Right, a vertical cut along the resonant
direction through the middle of the patch, with the ground plane and the
patch as the heavy lines: the same half wave appears in the substrate, and
the field that escapes past the two patch edges is the fringing that
radiates.}
\label{fig:field}
\end{figure}
...
stage & time & share \\
\midrule
mesh, assembly and matrix build & 16.4 ms & once \\
symbolic analysis & 5.6 ms & once \\
numeric factorization & 270 ms & per frequency \\
triangular solve & 3.7 ms & per frequency and port \\
\bottomrule
\end{tabular}
