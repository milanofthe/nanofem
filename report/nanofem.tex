...
\(1.57\) mm substrate of \(\varepsilon_r = 2.2\), terminated by a
perfectly matched layer and meshed by Gmsh into
\(37676\) tetrahedra and \(41468\) unknowns.

\begin{figure}[H]
\centering
\begin{tikzpicture}
\input{data/mesh3d.tikz}
\end{tikzpicture}
\caption{The mesh, cut in half at \(y = 50\) mm and drawn back to front with
opaque faces, inside the outline of the full domain. The thin layer at the
bottom is the substrate, the block above it is the air region, and the
shaded ring and cap are the perfectly matched layer. The element size follows a distance field on the patch and the feed,
growing from \(2\) mm at the metal to \(5.5\) mm at the outer corners, which
keeps the aspect ratio bounded through the transition out of the
\(1.57\) mm substrate.}
\label{fig:mesh}
\end{figure}
...
at \(2.50\) GHz, against a design value of \(2.45\) GHz. A mesh coarsened
by a factor of one and a half puts it at \(2.48\) GHz, so the
discretization error is still the largest term.}
\label{fig:patch}
\end{figure}
...
\centering
\begin{tikzpicture}
\input{data/slice3d.tikz}
\end{tikzpicture}
\caption{Contours of the electric field magnitude at the resonance, on a
horizontal cut through the middle of the substrate, at \(-11\), \(-7\),
\(-4\) and \(-2\) dB below the peak of the cut. The view, the scale and
the outlined box are those of figure~\ref{fig:mesh}, so the two are the
same scene: the box is the meshed domain, and the plane stands off it by the
thickness of the absorbing layer. The heavy rectangle is the patch. The
field is confined under the patch and forms the half wave along \(y\) that
sets the resonant length, with maxima at the two radiating edges and a null
across the middle. What escapes past those edges is the fringing that
radiates.}
\label{fig:field}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Runtime by stage on the reference antenna, on one core of an
Apple M3. The totals are wall clock, the split between stages comes from a
sampling profile of the same run. The factorization is 98 percent of the
work per frequency, so the fill of the ordering sets the runtime and the
assembly does not. A sweep runs its frequencies in parallel, one
factorization each, which is why the memory and not the arithmetic caps the
thread count.}
\label{tab:time}
\end{table}
...
\(4.8\) mm strip on the same \(1.57\) mm substrate, \(44\) mm long, in a
\(20\) mm wide enclosure with \(8\) mm of air above the substrate. The
...
strip. The mesh grades from \(0.6\) mm on the strip to \(4\) mm at the
shield, \(22775\) tetrahedra and \(23439\) unknowns, and the sweep of 37
frequencies from \(1\) to \(10\) GHz takes \(12.4\) s on eight threads.

\begin{figure}[H]
\centering
\begin{tikzpicture}
\input{data/line3d.tikz}
\end{tikzpicture}
\caption{Contours of the electric field magnitude at \(5\) GHz on a
transverse cut halfway along the line, at \(-21\), \(-15\), \(-9\) and
\(-3\) dB below the peak of the cut. The heavy lines are the strip. The
field is confined to the substrate under the strip and fringes past its two
edges, and it is \(30\) dB down at the side walls and the lid.}
\label{fig:line}
\end{figure}
...
\begin{tikzpicture}
\begin{axis}[bwaxis, width=0.72\textwidth, height=0.34\textwidth,
  xlabel={frequency / GHz}, ylabel={dB}, legend pos=south west]
...
\end{axis}
\end{tikzpicture}
\caption{Reflection, solid, and transmission, dashed, of the line.
Reflection stays below
\(-12\) dB from \(1\) to \(10\) GHz apart from a peak at \(7\) GHz,
where \(|S_{11}|\) reaches \(-3.5\) dB and \(|S_{21}|\) falls to
\(-2.7\) dB. Narrowing the enclosure from \(20\) to \(16\) mm removes
that peak and puts a similar one at \(9\) GHz, so it belongs to the box and
not to the line.}
\label{fig:lines}
\end{figure}
