...
resonates at \(2.45\) GHz on its default mesh and at \(2.50\) GHz on a mesh
refined by a factor of one and a half. Mesh convergence has to be established by
...
gives \(1.72\) million nonzeros in \(L\) against \(17.2\) million in the
...
against the wavelength. Equilibration gains a factor of about \(2.5\) over
the whole low frequency branch and is neutral in the operating band.}
\label{fig:cond}
\end{figure}
...
\(9418\) tetrahedra and \(9847\) unknowns.

\begin{figure}[H]
\centering
\begin{tikzpicture}[font=\footnotesize]
\begin{axis}[bwaxis, grid=none, width=0.92\textwidth, height=0.40\textwidth,
  xlabel={$y$ / mm}, ylabel={$z$ / mm}, enlargelimits=0.02,
  xmin=0, xmax=100, ymin=0, ymax=35]
\addplot[black, line width=0.2pt] table[x=x, y=y] {data/mesh_cut.dat};
\addplot[black, line width=1.1pt] coordinates {(31.6,1.57) (68.4,1.57)};
\end{axis}
\end{tikzpicture}
\caption{The tetrahedral mesh cut by the plane \(x = 45\) mm, the same plane
as the right hand field map. The substrate is the flat layer of elements
below \(z = 1.57\) mm and the patch is the heavy line on top of it. The
element size is set by a distance field on the patch and the feed, growing
from \(3\) mm at the metal to \(11\) mm at the far corners of the air box,
so the aspect ratio stays bounded through the transition out of the
substrate. A uniform maximum size instead produces slivers spanning the air
region and moves the resonance by two percent.}
\label{fig:mesh}
\end{figure}
...
at \(2.45\) GHz on this mesh. It is not converged: refining the mesh moves
it to \(2.50\) GHz, and part of the remaining drift is the first order
...
\begin{axis}[bwaxis, grid=none, width=0.45\textwidth, height=0.42\textwidth,
  view={0}{90}, xlabel={$x$ / mm}, ylabel={$y$ / mm},
  title={\footnotesize substrate, $z = h/2$, floor $-20$ dB},
  enlargelimits=false, axis on top, colormap={gray}{gray(0cm)=(1); gray(1cm)=(0.05)},]
\addplot3[surf, shader=interp, mesh/cols=90] table[x=x, y=y, z=e] {data/field_xy.dat};
\addplot3[black, line width=0.25pt] table[x=x, y=y, z expr=2] {data/cont_xy.dat};
\addplot3[black, line width=0.7pt] coordinates
  {(20.8,31.6,3) (69.2,31.6,3) (69.2,68.4,3) (20.8,68.4,3) (20.8,31.6,3)};
\end{axis}
\end{tikzpicture}\hfill
\begin{tikzpicture}[font=\footnotesize, baseline]
\begin{axis}[bwaxis, grid=none, width=0.45\textwidth, height=0.42\textwidth,
  view={0}{90}, xlabel={$y$ / mm}, ylabel={$z$ / mm},
  title={\footnotesize cut at $x = 45$ mm, floor $-25$ dB},
  enlargelimits=false, axis on top, colormap={gray}{gray(0cm)=(1); gray(1cm)=(0.05)},]
\addplot3[surf, shader=interp, mesh/cols=90] table[x=x, y=y, z=e] {data/field_xz.dat};
\addplot3[black, line width=0.25pt] table[x=x, y=y, z expr=2] {data/cont_xz.dat};
\addplot3[black, line width=0.9pt] coordinates {(22,0,3) (78,0,3)};
\addplot3[black, line width=0.9pt] coordinates {(31.6,1.57,3) (68.4,1.57,3)};
\end{axis}
\end{tikzpicture}
\caption{Magnitude of the electric field at the resonance, \(2.45\) GHz,
with the port driven, on a decibel scale relative to the peak of each cut
and contours at \(-11\), \(-7\), \(-4\) and \(-2\) dB of the range. Left, a horizontal cut
through the middle of the substrate with the patch outline drawn: the field
is confined under the patch and forms the half wave along \(y\) that sets the
resonant length, with maxima at the two radiating edges and a null across the
middle. Right, a vertical cut along the resonant direction through the middle
of the patch, with the ground plane and the patch as the heavy lines: the
same half wave appears in the substrate, and the field that escapes past the
two patch edges is the fringing that radiates.}
\label{fig:field}
\end{figure}
...
stage & time & share \\
\midrule
mesh, assembly and matrix build & 13.4 ms & once \\
symbolic analysis & 4.5 ms & once \\
numeric factorization & 229 ms & per frequency \\
triangular solve & 2.9 ms & per frequency and port \\
\bottomrule
\end{tabular}
\caption{Runtime by stage on the reference antenna. The factorization is
99 percent of the work per frequency, so the fill of the ordering sets the
runtime and the assembly does not.}
\label{tab:time}
\end{table}
...
Second order elements are the largest entry. They carry twenty unknowns per tetrahedron instead of six: the
